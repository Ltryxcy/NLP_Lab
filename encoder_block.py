...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, emb_size, query_key_size, value_size, hidden_size, head_num):
        super().__init__()
        # 多头注意力
        self.multihead_attn = MultiHeadAttention(emb_size, query_key_size, value_size, head_num)
        # 线性层，将多头注意力模块的输出调整为与输入嵌入大小相同的尺寸。
        self.z_linear = nn.Linear(head_num * value_size, emb_size)
        # 层归一化，按last dim做norm
